<template>
    <AppLayout>
        <Head :title="`${team.team_name} - Euroleague Stats`" />

        <div class="mb-6">
            <Link href="/teams" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 mb-4 inline-block transition-colors">
                ← Back to Teams
            </Link>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6 transition-colors duration-200">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div class="text-center">
                    <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ (stats.avg_points || 0).toFixed(1) }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">PPG</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ (stats.avg_rebounds || 0).toFixed(1) }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">RPG</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">{{ (stats.avg_assists || 0).toFixed(1) }}</div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <div class="text-center">
                    <div class="text-xl font-semibold text-gray-900 dark:text-white">{{ (stats.avg_field_goals || 0).toFixed(1) }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400">FG Made Per Game</div>
                </div>
                <div class="text-center">
                    <div class="text-xl font-semibold text-gray-900 dark:text-white">{{ (stats.avg_steals || 0).toFixed(1) }}</div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        <tr v-for="gameStat in recentGames" :key="gameStat.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <td class="px-4 py-3 text-sm">
                                <Link :href="`/games/${gameStat.game_id}`" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 transition-colors">
